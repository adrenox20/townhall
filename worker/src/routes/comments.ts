...
commentRoutes.post(
  '/issues/:id/comments',
  requireAuth,
  rateLimit('comment', 20, 3600),
  zValidator('json', commentSchema),
  async (c) => {
    const input = c.req.valid('json');
    const user = c.get('user');
    if (input.is_internal_note && !isAdmin(user.role)) return fail(c, 403, 'Only dept admins can add internal notes');
    if (input.is_official_update && !isAdmin(user.role)) return fail(c, 403, 'Only dept admins can post official updates');
    ...
    await c.env.DB.batch([
      ...
      c.env.DB.prepare(
        'INSERT INTO notifications (id, user_id, type, issue_id, comment_id, message) VALUES (?, ?, ?, ?, ?, ?)'
      ).bind(
        id('not'),
        issue.author_id,
...
commentRoutes.patch(
  '/comments/:id',
  requireAuth,
  zValidator('json', z.object({ body: z.string().min(2) })),
  async (c) => {
...
commentRoutes.post('/comments/:id/pin', requireAuth, requireRole('dept_admin'), async (c) => {
