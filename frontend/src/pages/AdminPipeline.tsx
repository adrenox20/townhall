...
// Admin pipeline shows the post-validation flow
const columns = ['validated', 'under_review', 'accepted', 'in_progress', 'resolved', 'closed'];

const columnColors: Record<string, { border: string; bg: string; dot: string }> = {
  validated: { border: 'border-teal-500/30', bg: 'bg-teal-500/5', dot: 'bg-teal-400' },
...
export function AdminPipeline() {
  ...
  const mutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) =>
      endpoints.updateStatus(id, { status, note: `Moved to ${statusLabels[status] ?? status} via pipeline board` }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['adminIssues'] }); setMovingId(null); toast.success('Status updated'); },
    onError: (error) => { setMovingId(null); toast.error(error.message); }
  });

  function onDragStart(event: DragStartEvent) {
    setActiveIssue(data?.find(i => i.id === String(event.active.id)) ?? null);
  }
  ...
  // Only show post-validation issues in admin pipeline
  const filteredData = data?.filter(i => columns.includes(i.status)) ?? [];

  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white font-display">Admin Pipeline Board</h1>
      ...
      <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd} onDragCancel={() => setActiveIssue(null)}>
        <div className="grid gap-3 lg:grid-cols-6 overflow-x-auto pb-2" style={{ minHeight: '450px' }}>
          {columns.map(column => {
            const issues = filteredData.filter(issue => issue.status === column);
            return (
...
function PipelineColumn({ id, count, children }: { id: string; count: number; children: React.ReactNode }) {
  ...
  const colors = columnColors[id] ?? columnColors.validated;
  return (
    <div ref={setNodeRef} className={`min-h-[350px] rounded-xl border p-3 transition-all duration-200 ${isOver ? 'border-indigo-500/50 bg-indigo-500/5 ring-1 ring-indigo-500/20' : `${colors.border} ${colors.bg}`}`}>
...
function DraggableCard({ issue, isMoving, isDragActive }: { issue: Issue; isMoving: boolean; isDragActive: boolean }) {
  ...
  return (
    <div ref={setNodeRef} {...listeners} {...attributes} style={style} className={`w-full rounded-lg border border-white/[0.08] bg-[#1a1d2e] p-3 text-left transition-none cursor-grab active:cursor-grabbing hover:border-indigo-500/30 ${isDragActive ? 'opacity-30 scale-95' : ''} ${isMoving ? 'opacity-50 animate-pulse' : ''}`}>
      ...
      {isMoving && <div className="mt-2 flex items-center gap-1 text-[10px] text-indigo-300"><Loader2 size={10} className="animate-spin" /> Moving...</div>}
    </div>
  );
}
