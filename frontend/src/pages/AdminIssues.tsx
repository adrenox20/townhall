...
import { Search, ExternalLink, ChevronDown, ChevronRight, Trash2, Pin, MessageSquare, Loader2, AlertTriangle, X } from 'lucide-react';
import { endpoints } from '../lib/api';
import { Badge, statusClass, statusLabels, priorityClass } from '../lib/badges';
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">{data?.length ?? 0} total issues · Click a row to moderate comments</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select className="field w-auto min-w-[150px]" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}>
...
function IssueRow({ issue, expanded, onToggle }: { issue: Issue; expanded: boolean; onToggle: () => void }) {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ status }: { status: string }) => endpoints.updateStatus(issue.id, { status, note: `Changed to ${statusLabels[status]} by admin` }),
    ...
    <div className="panel overflow-hidden">
      ...
      >
        ...
        <Link
          ...
          className="p-1.5 rounded-lg hover:bg-white/[0.06] text-slate-400 hover:text-indigo-300 transition-colors"
        >
      ...
      {expanded && (
        <div className="border-t border-white/[0.06] p-4 bg-white/[0.01] space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {['submitted', 'under_review', 'accepted', 'in_progress', 'resolved', 'closed', 'wont_fix'].map(s => (
              <button
                key={s}
                ...
                className={`text-[10px] px-2 py-1 rounded-md border transition-colors ${
                  issue.status === s
                    ...
                    : 'text-slate-400 border-white/[0.08] hover:border-white/[0.2] hover:text-white disabled:opacity-40'
                }`}
              >
                {statusLabels[s] ?? s}
              </button>
...
function CommentsPanel({ issueId }: { issueId: string }) {
  ...
  const { data: comments, isLoading } = useQuery({
    ...
    queryFn: () => endpoints.issueComments(issueId),
  });

  const deleteMutation = useMutation({
    mutationFn: endpoints.deleteComment,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['issueComments', issueId] }); toast.success('Comment removed (inappropriate content deleted)'); },
    onError: (e) => toast.error(e.message),
  });
  ...
  if (!comments?.length) {
    return (
      <div className="text-xs text-slate-500 py-2 flex items-center gap-1.5">
        <MessageSquare size={12} /> No comments on this issue.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-xs text-slate-400 mb-2">
        ...
        <span className="text-slate-600">· Delete inappropriate content below</span>
      </div>
      {comments.map(comment => (
        <div key={comment.id} className={`flex items-start gap-3 p-3 rounded-lg border transition-colors ${
          comment.is_pinned ? 'border-amber-500/30 bg-amber-500/5' :
          comment.is_internal_note ? 'border-purple-500/20 bg-purple-500/5' :
          comment.is_official_update ? 'border-indigo-500/20 bg-indigo-500/5' :
          'border-white/[0.05] bg-white/[0.01]'
        }`}>
          {/* Avatar */}
          <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-[9px] text-white font-semibold shrink-0 mt-0.5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-medium text-slate-200">{comment.author_name ?? 'Unknown'}</span>
              ...
              {comment.is_internal_note ? <span className="text-[9px] px-1.5 py-0.5 rounded bg-purple-500/20 text-purple-300 font-medium">Internal</span> : null}
            ...
            <p className="text-xs text-slate-300 leading-relaxed line-clamp-3">{comment.body}</p>
          </div>
          ...
          <div className="flex items-center gap-1 shrink-0">
            <button
              ...
              className={`p-1.5 rounded-md transition-colors ${comment.is_pinned ? 'text-amber-400 bg-amber-500/10' : 'text-slate-500 hover:text-amber-400 hover:bg-amber-500/10'}`}
            ...
            <button
              onClick={() => {
                if (confirm('Delete this comment? This removes inappropriate content from public view.')) {
