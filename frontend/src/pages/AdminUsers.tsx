...
import { Badge, roleClass, roleLabels } from '../lib/badges';

const roles = ['student', 'student_council', 'dept_admin', 'super_admin'] as const;
...
export function AdminUsers() {
  ...
  if (isLoading) return (
    <div className="space-y-4">
      <div className="skeleton h-8 w-48" /><div className="skeleton h-12 w-full" /><div className="skeleton h-64 w-full" />
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-400 bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2">
          ...
          <span>Users sign in with @rishihood.edu.in, then promote their role here</span>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <select className="field w-auto min-w-[160px]" value={roleFilter} onChange={e => setRoleFilter(e.target.value)}>
      ...
      <div className="panel overflow-hidden">
                ...
                <tr key={user.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="p-4">
                    <div className="relative">
                      <select value={user.role} onChange={e => roleMutation.mutate({ userId: user.id, role: e.target.value })} disabled={roleMutation.isPending}
                  ...
                  <td className="p-4 text-right">
                    <button onClick={() => banMutation.mutate({ userId: user.id, banned: !user.is_banned })} disabled={banMutation.isPending}
                      className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors ${user.is_banned ? 'bg-emerald-600/15 text-emerald-300 hover:bg-emerald-600/25 border border-emerald-500/25' : 'bg-red-600/15 text-red-300 hover:bg-red-600/25 border border-red-500/25'}`}>
        ...
        {filtered.length === 0 && <div className="p-8 text-center text-sm text-slate-400">No users match your filters.</div>}
      </div>
    </div>
  );
}
